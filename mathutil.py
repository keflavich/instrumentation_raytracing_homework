...
brackets a root of f and fnvals=(f(x1),f(x2)).
	...
	if x1==x2:
		...
		x1, x2 = float64(x2), float64(x1)
	f1, f2 = f(x1), f(x2)
	for j in range(max_iterations):
		while f1*f2 >= float64(0):  # not currently bracketed
			if abs(f1)<abs(f2):
				x1 = x1 + GOLDEN*(x1-x2)
			else:
				x2 = x2 + GOLDEN*(x2-x1)
...
find a root x of f using Ridder's method. Parameter fnvals=(f(x1),f(x2)) is optional.
	...
	if f1==float64(0):
		...
		return float64(x2)
	elif f1*f2>=float64(0):
		raise BracketingException("initial interval does not bracket a root")
	x4 = float64(123456789.)
	for j in range(max_iterations):
		x3 = (float64(x1)+float64(x2))/float64(2)
		f3 = f(x3)
		temp = f3*f3 - f1*f2
		x4, x4old = x3 + (x3-x1)*sgn(f1-f2)*f3/temp**.5, x4
		f4 = f(x4)
		if float64(f1)*float64(f4)<float64(0):  # x1 and x4 bracket root
			x2, f2 = x4, f4
